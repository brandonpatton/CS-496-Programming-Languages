...
and
  ...
  | Variant(tag,args) -> 
	TaggedVariantVal (tag, List.map(eval_expr en)  args)
  | Case(cond,branches) -> let tagv = eval_expr en cond in
      (match tagv with
      	| TaggedVariantVal(tag, args) -> 
            	let nb = List.find (fun (Branch(id,_,_)) -> id = tag) branches in
            	eval_expr (extend_env_list en (b_ids nb) args)  (b_expr nb)
      	| _ -> failwith"Not a Tagged Variant")
