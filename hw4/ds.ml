...
type exp_val =
  ...
  | RefVal of int
  | UnitVal of unit
  | PairVal of Ast.texpr*Ast.texpr
  | ListVal of Ast.texpr
  | TreeVal of Ast.texpr
and
  env =
  | EmptyEnv
...
let rec string_of_expval = function
  ...
  | PairVal (x,y) -> "PairVal (" ^ Ast.string_of_texpr x^ "," ^ Ast.string_of_texpr y^ ")"
  | ListVal l -> "ListVal (" ^Ast.string_of_texpr l^ ")"
  | TreeVal t -> "TreeVal (" ^Ast.string_of_texpr t^ ")"
  | UnitVal u -> "Unit"
