...
and
  ...
  | SetRef(e1,e2) ->
    ...
    UnitVal
  | Open(id,e) -> failwith "implement me!"
  | Debug ->
...
and
  add_module_definition (en:env) (AModBody vdefs):env =
    snd @@ LIst.fold_left (fun (globalEnv,localEnv) (AValDef(id,e)) -> 
	let ev = eval_expr globalEnv e in (ExtendEnv(id,ev,globalEnv),ExtendEnv(id,ev,localEnv)) (en, empty_env ()) vdefs)
and
  add_module_definitions (en:env) (ms:mdecl list):env =
    List.fold_left (fun acc (AMod(id,iface,body)) ->
	 ExtendEnvMod(id,add_module_definition acc body,acc)) en ms
 and
